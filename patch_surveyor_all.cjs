...
code = code.replaceAll(
  /if \(user.role === "District LAO" && \(user.state !== proj.state \|\| user.district !== proj.district\)\) return res.status\(403\).json\(\{ error: "Unauthorized jurisdiction" \}\);/g,
...
console.log('Patched surveyor all');
